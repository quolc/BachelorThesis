...
We recorded videos and embedded watermarks on them with various watermarking settings, and measured the correct detection rates (CDRs) of watermarks in each of them.
...
From the result, we can conclude that 10 ms was the optimal value for the length of a data frame.
...
% 結果
The result shown in Figure \ref{fig:eval_reli_pyld} demonstrates that CDR was slightly decreased with payload length increasing, however retained above 95\% at all payload lengths and situations.
